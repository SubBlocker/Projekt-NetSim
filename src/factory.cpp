...
ParsedLineData parse_line(std::string& line) {
    ...
    try {
        parsed_data.element_type = element_types.at(tokens.at(0));

        std::for_each(std::next(tokens.cbegin()), tokens.cend(),
                      [&](const std::string& parameter_str) {
                          auto key_value = character_split(parameter_str, '=');
                          parsed_data.parameters[key_value.at(0)] = key_value.at(1);
                      });
    } catch (...) {
        throw std::exception();
    }
...
static PackageQueueType get_package_queue_type(std::string& package_queue_type_str) {
...
static void link_nodes(Factory& factory, const std::map<std::string, std::string>& parameters) {
    enum class NodeType { RAMP, WORKER, STORE };
    ...
    NodeType src_node_t = str_node_type.at(src_param.at(0));
    ElementID src_node_id = std::stoi(src_param.at(1));

    auto dest_param = character_split(dest_str, '-');
    NodeType dest_node_t = str_node_type.at(dest_param.at(0));
    ElementID dest_node_id = std::stoi(dest_param.at(1));
    ...
    while (std::getline(is, line)) {
        ...
        switch (parsed.element_type) {
            ...
            case ElementType::LINK: {
                link_nodes(factory, parsed.parameters);
                break;
            }
        }
    }
...
static void link_stream_fill(std::stringstream& link_stream,
    ...
    std::for_each(prefs.cbegin(), prefs.cend(),
                  [&](const std::pair<IPackageReceiver*, double>& key_value) {
                      ...
                      std::string receiver_type_str =
                          (receiver_type == ReceiverType::WORKER) ? "worker" : "store";
