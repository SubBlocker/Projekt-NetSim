...
protected:
    void push_package(Package&& moved_package) {
        buffer_ = std::move(moved_package);
    };
...
public:
    Ramp(ElementID id, TimeOffset di): PackageSender(), id_(id), di_(di), t_(), buffer_(std::nullopt) {}
